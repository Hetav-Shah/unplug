...
# -------------------------------------------------------
# Quaternion helper (continuous rotation)
# -------------------------------------------------------
...
def main(args=None):
    ...
    CAP_QUAT = [0.9040584664694167, -0.4265039161201177, -0.026731102818722683, 0.0076254102358457206]
    ...
    try:
        ...
        for i in range(N_TURNS):
            ...
            robotB.get_logger().info(f"Rotation {i+1}/{N_TURNS}")

            current_quat = rotate_about_z_continuous(
                current_quat, ROT_DEG
            )

            move_and_wait(CAP_POS, current_quat, f"ROTATE {i+1}")

            robotB.publish_zero_velocity()
            gripper.open_gripper(width=0.08)
            time.sleep(0.4)

            move_and_wait(CAP_POS, CAP_QUAT, f"RESET {i+1}")
